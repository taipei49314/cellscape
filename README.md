...
此命令執行隨包的 Atlas 16 項、Loop 7 項、模型行為契約 11 項與最終觀察邊界 4 項（共 38 項）；**不是完整瀏覽器或生物學驗證**。模型行為契約涵蓋貧血上限、Q10、CO₂ 鉗位與帳目、參數界限，以及凍結檔雜湊守門——**目前只實守 model.js／render.js／engine.js／data.js 四檔**，`core.js`／`content.js`／`tour.js` 因具名解凍已列豁免，所以 CI 綠不等於沒碰凍結檔。
