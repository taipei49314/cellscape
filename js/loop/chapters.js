...
(function (global) {
  ...
  const DEFAULTS = { lungSupply: 0.85, flowSpeed: 1.0, tissueDemand: 0.5, anemia: 0, temperature: 37.0, perfusion: 1.0, altitudeM: 0 };
  /* 這兩支滑桿與藥物預設才會建立 A/B 基線（js/loop/main.js）；
     提示語必須照實寫，不能說「任一次介入都會建立」。 */
  const BRANCHING_KEYS = ['lungSupply', 'anemia', 'altitudeM'];
  ...
  const CHAPTERS = [
    ...
    },
    {
      ...
      gates: [
        {
          id: 'C1',
          zh: '建立 A/B 基線分支（動「肺端供氧」、「貧血程度」或「海拔」滑桿，或按任一個藥物預設）',
