  ...
  const M = (CSL.Main = {
    ...
    _bind() {
      ...
      const DRUG_PRESETS = {
        ...
        baseline: { params: { lungSupply: 0.85, flowSpeed: 1, tissueDemand: 0.5, anemia: 0, temperature: 37, perfusion: 1 }, note: '藥物介入（模型）：參數回復基準值（含貧血、體溫與灌流）。' },
      };
    ...
    _syncParamsUI() {
      for (const key of ['lungSupply', 'flowSpeed', 'tissueDemand', 'anemia', 'temperature', 'perfusion']) {
