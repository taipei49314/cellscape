    ...
    _bind() {
      ...
      document.querySelectorAll('#paramPanel input[type=range]').forEach((el) => {
        el.addEventListener('input', () => {
          ...
          $('pv_' + key).textContent = key === 'altitudeM' ? String(Math.round(v)) : ((key === 'fluidRate' || key === 'glucoseIntake') ? v.toFixed(3) : v.toFixed(2));
          CSL.queueCommand(this.world, { kind: 'setParam', key, value: v, source: 'user' });
          if (key === 'lungSupply' || key === 'anemia' || key === 'altitudeM' || key === 'fluidRate' || key === 'infection' || key === 'glucoseIntake') this.ensureBranch();   // 首次介入建立 A/B
        });
      });
      ...
      const DRUG_PRESETS = {
        ...
        meal: { params: { glucoseIntake: 0.02 }, note: '藥物介入（模型）：進食——連續碳水攝入 0.02／tick（血糖指數上升、胰島素分泌、組織攝取加速）。模型單位，非用藥建議。' },
        fasting: { params: { glucoseIntake: 0 }, note: '藥物介入（模型）：禁食——碳水攝入停止（血糖指數隨組織攝取回落）。模型單位，非用藥建議。' },
        ...
        baseline: { params: { lungSupply: 0.85, flowSpeed: 1, tissueDemand: 0.5, anemia: 0, temperature: 37, perfusion: 1, altitudeM: 0, fluidRate: 0, infection: 0, glucoseIntake: 0 }, note: '藥物介入（模型）：參數回復基準值（含貧血、體溫、灌流、海拔、補水、感染與攝入）。' },
      };
      document.querySelectorAll('#drugPanel .drug').forEach((el) => {
        el.addEventListener('click', () => {
          ...
          for (const [key, value] of Object.entries(preset.params)) {
            ...
            $('pv_' + key).textContent = key === 'altitudeM' ? String(Math.round(Number(value))) : ((key === 'fluidRate' || key === 'glucoseIntake') ? Number(value).toFixed(3) : Number(value).toFixed(2));
          }
          this.ensureBranch();
