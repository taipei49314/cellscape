...
(function (global) {
  ...
  CSL.Content = {
    ...
    claims: {
      ...
      /* 模型近似（對照本版實作核對） */
      'C-model-load':     { text: '本模型以 0–1 負載欄位近似攜帶狀態，無逐分子結合/解離。', contentType: 'model_assumption', sourceIds: [], implRef: 'js/loop/core.js（entity.load、交換規則）', reviewStatus: 'verified-implementation', supportedLimit: 'MODEL_VERSION 0.6.0；非 SpO₂。' },
      'C-model-48':       { text: '48 顆 RBC 為固定抽樣代表，數量不代表全身總量。', contentType: 'model_assumption', sourceIds: [], implRef: 'js/loop/core.js ENTITY_COUNT', reviewStatus: 'verified-implementation', supportedLimit: 'MODEL_VERSION 0.6.0。' },
      'C-model-aggregate':{ text: '肺端/組織端交換由聚合界面規則處理，沒有個別上皮/內皮細胞實體。', contentType: 'model_assumption', sourceIds: [], implRef: 'js/loop/model.js（edges exchange）', reviewStatus: 'verified-implementation', supportedLimit: 'MODEL_VERSION 0.6.0。' },
      'C-model-lungsupply': { text: 'lungSupply 參數同時作用於外部輸入項與肺端交換項，不是吸入氧濃度。', contentType: 'model_assumption', sourceIds: [], implRef: 'js/loop/core.js step() 輸入項與 lung 交換', reviewStatus: 'verified-implementation', supportedLimit: 'MODEL_VERSION 0.6.0。' },
      'C-model-flowspeed':  { text: 'flowSpeed 等比縮放各邊跨越時間，不是心率或血壓。', contentType: 'model_assumption', sourceIds: [], implRef: 'js/loop/model.js baseTicks', reviewStatus: 'verified-implementation', supportedLimit: 'MODEL_VERSION 0.6.0；與 perfusion 分工——flowSpeed 只改跨越時間（多快繞一圈），perfusion 只改組織端卸載通量（這個床分到多少血流），兩者都不是 mmHg 血壓。' },
      'C-model-tissuedemand': { text: 'tissueDemand 同時作用於組織端交換與使用項，非單一真實生理量。', contentType: 'model_assumption', sourceIds: [], implRef: 'js/loop/core.js step() 組織交換與使用', reviewStatus: 'verified-implementation', supportedLimit: 'MODEL_VERSION 0.6.0。' },
      ...
      'C-model-perfusion':{ text: 'perfusion 參數代表流向這個組織床的血流比例，只乘組織端卸載通量；本模型只有一個組織床，其餘血管床未建模。', contentType: 'model_assumption', sourceIds: [], implRef: 'js/loop/core.js step() 組織端 flux × perfusion', reviewStatus: 'verified-implementation', supportedLimit: 'MODEL_VERSION 0.6.0；不是 mmHg 血壓、不是心輸出量，也不模擬其他器官的搶流。' },
