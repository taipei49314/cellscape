...
(function (global) {
  ...
  CSL.Content = {
    ...
    claims: {
      ...
      /* 模型近似（對照本版實作核對） */
      'C-model-load':     { text: '本模型以 0–1 負載欄位近似攜帶狀態，無逐分子結合/解離。', contentType: 'model_assumption', sourceIds: [], implRef: 'js/loop/core.js（entity.load、交換規則）', reviewStatus: 'verified-implementation', supportedLimit: 'MODEL_VERSION 0.5.0；非 SpO₂。' },
      'C-model-48':       { text: '48 顆 RBC 為固定抽樣代表，數量不代表全身總量。', contentType: 'model_assumption', sourceIds: [], implRef: 'js/loop/core.js ENTITY_COUNT', reviewStatus: 'verified-implementation', supportedLimit: 'MODEL_VERSION 0.5.0。' },
      'C-model-aggregate':{ text: '肺端/組織端交換由聚合界面規則處理，沒有個別上皮/內皮細胞實體。', contentType: 'model_assumption', sourceIds: [], implRef: 'js/loop/model.js（edges exchange）', reviewStatus: 'verified-implementation', supportedLimit: 'MODEL_VERSION 0.5.0。' },
      'C-model-lungsupply': { text: 'lungSupply 參數同時作用於外部輸入項與肺端交換項，不是吸入氧濃度。', contentType: 'model_assumption', sourceIds: [], implRef: 'js/loop/core.js step() 輸入項與 lung 交換', reviewStatus: 'verified-implementation', supportedLimit: 'MODEL_VERSION 0.5.0。' },
      'C-model-flowspeed':  { text: 'flowSpeed 等比縮放各邊跨越時間，不是心率或血壓。', contentType: 'model_assumption', sourceIds: [], implRef: 'js/loop/model.js baseTicks', reviewStatus: 'verified-implementation', supportedLimit: 'MODEL_VERSION 0.5.0。' },
      'C-model-tissuedemand': { text: 'tissueDemand 同時作用於組織端交換與使用項，非單一真實生理量。', contentType: 'model_assumption', sourceIds: [], implRef: 'js/loop/core.js step() 組織交換與使用', reviewStatus: 'verified-implementation', supportedLimit: 'MODEL_VERSION 0.5.0。' },
      ...
      'C-model-co2':      { text: 'CO₂ 以 0–1 指數近似：組織隨使用量生產、肺端隨通氣排出；Bohr 效應以卸載倍率（0.75–1.35）近似。', contentType: 'model_assumption', sourceIds: [], implRef: 'js/loop/core.js co2 與 step() Bohr 項', reviewStatus: 'verified', supportedLimit: 'MODEL_VERSION 0.5.0；模型指數，非臨床酸鹼值。' },
      'C-model-temp':     { text: '體溫以 Q10 因子（2^((T−37)/10)）乘 O₂ 使用速率；CO₂ 生產隨使用量連動。', contentType: 'model_assumption', sourceIds: [], implRef: 'js/loop/core.js step() q10 項', reviewStatus: 'draft-pending', supportedLimit: 'MODEL_VERSION 0.5.0；工程近似，非體溫調節模型。' }
    },
