...
(function (global) {
  ...
  const Atlas = {
    ...
    renderMech() {
      ...
      if (this.activeCard === 'rbc') ['C-model-load', 'C-model-aggregate', 'C-model-lungsupply', 'C-model-flowspeed', 'C-model-tissuedemand', 'C-model-co2', 'C-model-temp', 'C-model-anemia', 'C-model-perfusion', 'C-model-rbc-turnover', 'C-model-altitude', 'C-model-volume', 'C-model-infection', 'C-model-wbc-8', 'C-model-glucose', 'C-model-insulin'].forEach((c) => claimIds.add(c));
      ...
      const rulesBlock = `
        ...
        <ul class="kps">
          ...
          <li>血量：血漿容積為獨立載體小帳——補水流由 fluidRate、流出為溫度衍生出汗；容積比 volFrac 乘移動流速與卸載通量，<0.70 觸發 volumeLow（模型 1.0.0；非脫水醫學）。</li>
          <li>免疫：感染嚴重度釘組織微血管——嗜中性球自骨髓池每 90 tick 招募一顆（上限 8）、抵組織微血管即外滲清除 0.125；WBC 不攜氧、不適用世代輪替（模型 1.1.0；非感染症醫學）。</li>
          <li>血糖—胰島素：連續攝入由 glucoseIntake 流入血糖升高指數（0–4.0，禁食基線 0 起始）；組織攝取＝基線項＋胰島素放大項，胰島素水位（0.30–1.0）對高值回應——≥0.55 分泌上升、回落衰退；glucoseHigh 遲滯閾值（<0.45 解除）（模型 1.2.0；非臨床血糖值）。</li>
        </ul>
