...
- 本地靜態開啟 `loop.html`（建議經 HTTP：`python -m http.server 8642`）。
- 入口：`index.html` 頂列「v0.2 Living Loop ↗」。
- `index.html` 的八個場景為**既有獨立場景**（r4 基線，未與新世界連動）；
...
- 灌流參數 `perfusion`（0.2–1.8，預設 1.0；0.6.0 新增）：只乘組織端卸載通量——代表流向這個組織床的血流比例。1.0＝0.5.1 行為；低灌流時氧留在血中（平均負載上升）而不是憑空消失，守恆帳結構不變。**不是** mmHg 血壓、**不是**心輸出量；本模型只有一個組織床，其餘血管床未建模，所以「再分配」只表現為這個床拿到的比例。與 `flowSpeed` 分工：後者只改跨越時間。
...
- CO₂ 指數（0–1，穩態 0.30；0.4.0 新增）：組織隨 O₂ 使用量生產（RQ 0.8、尺度 4）、肺端隨通氣排出（係數 0.34）；上限鉗位差額記入 retained。Bohr 效應以組織卸載倍率近似（鉗位 0.75–1.35；穩態時＝1，即上式卸載項另乘 bohr）。CO₂ 自帶生產/排出/保留帳（每 30 tick 檢查），**不與 O₂ 守恆帳混用**；≥0.70 觸發 `co2BloodHigh` 閾值事件（遲滯 <0.60 解除）。指數與模型 pH 換算（7.40 − 0.35×(blood−0.30)/0.70）皆為模型值，非臨床量。
...
- Gate C 進度：速率曲線圖已於 T-319 交付（見上方速率曲線段）；血紅素放大視角
  ...
  主張、其餘標示意）。**仍未做**：章節 B/C 學習控制器、其餘卡片內容的編輯審核
  （現為待審草稿）。
