...
- 學習成效：未做人類評估，**不宣稱**。原五人小樣本腳本（TRY.md）已於 2026-09-15 經人類裁決移除。
- 第一輪八分鐘口頭因果以底部「肺泡水位／平均負載」為準（拉低肺端供氧後這兩項會先分開）。組織庫存傳導較慢，不是這輪必看項；長跑下組織仍可分開，下方模型契約列不變。
...
- 血量容積（1.0.0 新增）：新 compartment `volume`（0–5.0，初始等容）；補水參數 `fluidRate`（0–0.02／tick）流入、溫度衍生出汗 0.0002×max(0, temperature−37) 流出；容積比 volFrac 乘移動流速與組織端卸載通量，<0.70 觸發 `volumeLow`（≥0.80 解除）。容積獨立小帳 intake/sweat 每 30 tick 殘差檢查（中立性契約 volume-neutral-at-default 守門）。聚合模型指數，非脫水醫學、非輸液建議。
- 免疫招募（1.1.0 新增）：感染參數 `infection`（0–1）釘組織微血管；嗜中性球（kind `wbc`，同時上限 8）自 VEIN_SYS 每 90 tick 招募、抵 TISSUE_CAP 即外滲，每次清除 0.125、歸零發感染清除事件；WBC 負載恆 0、不適用 RBC 世代輪替；免疫狀態入 digest（中立性契約 infection-neutral-at-0 守門）。招募節奏為工程近似，非感染症醫學、非用藥建議。
- 血糖—胰島素（1.2.0 新增）：攝入參數 `glucoseIntake`（0–0.02／tick，預設 0）流入新 compartment `glucose`（0–4.0，相對禁食基線的升高指數，0 起始）；組織攝取率＝胰島素非依賴基線項＋胰島素放大項；胰島素狀態 `insulin`（0.30 禁食基線–1.0）對指數 ≥0.55 每 tick 分泌上升、回落向基線衰退；`glucoseHigh` 閾值事件（≥0.55 觸發、<0.45 解除遲滯）＋`insulin_response` system 事件；血糖小帳 intake/uptake 每 30 tick 殘差檢查（中立性契約 glucose-neutral-at-default 守門）。模型指數，非臨床血糖值、非糖尿病醫學、非飲食建議。
...
- RBC 世代輪替（0.8.0／T-329 F3；T-400 補帳）：滿 `RBC_MAX_LOOPS` 圈退役，殘餘 load 記入 `ledger.expelled`（離開抽樣體），同槽於肺端替換為負載 0.12 的新球並記入 `ledger.input`；總量固定 48。示意輪替，非 120 天生理壽命校準。WBC 負載恆 0，進出不進氧帳。
...
- **章節 B/C 學習控制器（T-321 刀 K2）**：`js/loop/chapters.js`。倉內原本沒有章節
  ...
  當場變成一片缺資料。學習成效仍須人類的五人小樣本驗收（Gate D，尚未執行）。【補記 2026-09-15：五人小樣本方案經人類裁決移除（TRY.md 已刪）；學習成效未做人類評估、不宣稱。】
